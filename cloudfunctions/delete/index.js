...
//以上全部data内容
  ...
  if(id1==""){
    ...
    cloud.database().collection(ku).doc(id).get().then((res)=>{
      ...
      var good_detail=res.data.good_detail
      var total=0
      console.log("total1：",total)
      console.log("chang:",good_detail.huifunr.length)
      if(liuyan==false){
        for(var i=0;i<good_detail.huifunr.length;i++){
          var dd=good_detail.huifunr[i]
          if(dd.time==time && dd.plrid==id0){
            console.log("nbbb",dd.huifushu)
  
            if(dd.huifushu==undefined){
              total=-1
            }else{
              total=dd.huifushu+1
              total=-total
      ...
      cloud.database().collection(ku).doc(id).update({
        data: {
          'good_detail.huifunr':_.pull({
            'time':_.eq(time),
            'plrid':_.eq(id0)
          }),
          'good_detail.huifushu':_.inc(total)
        }
      })
    })
    ...
    cloud.database().collection(ku).where({
      _id:id,
      "good_detail.huifunr.plrid":id0,
      "good_detail.huifunr.time":time,
    }).update({
      data: {
        'good_detail.huifunr.$.huifushu':_.inc(-1),
        'good_detail.huifunr.$.huifu':_.pull({
          time:_.eq(time1),
          plrid:_.eq(id1)
        }),
